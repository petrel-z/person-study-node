...
- **AI赋能**：利用大模型（支持 DeepSeek/文心一言/豆包）提供个性化洞察和建议
...
**AI 配置说明：**
- 用户从 3 个提供商中选择一个（DeepSeek/文心一言/豆包）
...
#### 4.2.3 状态管理 (`/src/queries`)
```
queries/
├── useSystems.ts           # 系统数据查询
├── useInsights.ts          # AI 洞察查询
├── useLogs.ts              # 系统日志查询
├── useJournal.ts           # 用户日记查询
├── useUser.ts              # 用户配置查询
└── mutations.ts            # 修改操作（更新评分、添加日志等）
```
...
#### 4.2.4 Hooks (`/src/hooks`) - 与 queries 同级
```
hooks/
├── useIPC.ts               # IPC 通信封装
├── usePinLock.ts           # PIN 锁定状态管理
└── useDatabaseHealth.ts    # 数据库健康监控
```
...
```
backend/
├── main.py                 # 应用入口（支持 --dev HTTP 模式和 IPC 模式）
├── config.py               # 配置管理
...
│   ├── log.py              # 系统日志 API
│   ├── journal.py          # 用户日记 API（新增）
│   ├── ai_config.py        # AI 配置 API（新增）
│   └── user.py             # 用户配置 API
├── core/
│   ├── __init__.py
│   ├── security.py         # PIN 哈希与验证、API Key 加密
│   ├── database.py         # SQLite 连接与建表
│   ├── health.py           # 健康检查与监控
│   └── config.py           # 配置类定义
├── models/
│   ├── __init__.py
│   ├── user.py             # 用户数据模型（拆分为 profile + settings）
│   ├── system.py           # 系统数据模型（base + 8个专属表）
│   ├── log.py              # 系统日志数据模型
│   └── journal.py          # 用户日记数据模型（新增）
├── schemas/
│   ├── __init__.py
...
│   ├── journal.py          # 日记 Pydantic Schema（新增）
│   └── ai_config.py        # AI 配置 Schema（新增）
├── services/
│   ├── __init__.py
│   ├── ai_service.py       # AI 服务封装（支持 3 个提供商）
│   ├── insight_service.py  # 洞察生成逻辑
│   ├── encryption_service.py  # 加密服务（新增）
...
CREATE TABLE ai_config (
    user_id INTEGER PRIMARY KEY DEFAULT 1,               -- 关联用户ID，固定为1
    provider TEXT NOT NULL CHECK(provider IN ('deepseek', 'wenxin', 'doubao')),  -- AI提供商
